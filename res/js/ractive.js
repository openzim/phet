...
;(function (global, factory) {
  ...
  function camelize(string) {
    return string.replace(camel, function (s) {
  ...
  function decamelize(string) {
    return string.replace(decamel, function (s) {
  ...
  if (hasConsole) {
    ...
    var welcomeMessage =
      "You're running Ractive 0.8.1 in debug mode - messages will be printed to the console to help you fix problems and optimise your application.\n\nTo disable debug mode, add this line at the start of your app:\n  Ractive.DEBUG = false;\n\nTo disable debug mode when your app is minified, add this snippet:\n  Ractive.DEBUG = /unminified/.test(function(){/*unminified*/});\n\nGet help and support:\n  http://docs.ractivejs.org\n  http://stackoverflow.com/questions/tagged/ractivejs\n  http://groups.google.com/forum/#!forum/ractive-js\n  http://twitter.com/ractivejs\n\nFound a bug? Raise an issue:\n  https://github.com/ractivejs/ractive/issues\n\n"
  ...
  var deprecations = {
    ...
    render: {
      deprecated: 'init',
      message:
        'The "init" method has been deprecated ' +
  ...
  function build(ractive, keypath, value) {
    ...
    if (isObject(keypath)) {
      var loop = function (k) {
        if (keypath.hasOwnProperty(k)) {
          sets.push.apply(
            sets,
            gather(ractive, k).map(function (m) {
              return [m, keypath[k]]
            }),
          )
        }
    ...
    // set a single keypath
      ...
      sets.push.apply(
        sets,
        gather(ractive, keypath).map(function (m) {
          return [m, value]
        }),
      )
    }
  ...
  ModelBase.prototype.notifyUpstream = function notifyUpstream() {
    ...
    while (parent) {
      if (parent.patternObservers.length)
        parent.patternObservers.forEach(function (o) {
    ...
    function model(mdl, args) {
      ...
      if (!isArray(array)) {
        if (array === undefined) {
          ...
          var promise$1 = runloop.start(this, true).then(function () {
            return result$1
          })
          mdl.set(array)
          runloop.end()
  ...
  function add$1(keypath, value) {
    ...
    return set(
      this.ractive,
      build$1(this, keypath, value).map(function (pair) {
        var model = pair[0],
          val = pair[1],
          value = model.get()
  ...
  function link(source, dest) {
    var there = findModel(this, source).model,
      here = findModel(this, dest).model
  ...
  ReferenceResolver.prototype.rebinding = function rebinding(next, previous) {
    ...
    if (next)
      runloop.scheduleTask(function () {
        return next.addUnresolved(this$1.keys[0], this$1)
      })
  }
  ...
  ReferenceResolver.prototype.unbind = function unbind() {
    ...
    this.contexts.forEach(function (c) {
      return c.removeUnresolved(this$1.keys[0], this$1)
    })
  }
    ...
    if (!this.dirty || this.changed.length) {
      ...
      if (!this.changed.length) {
        ...
        var ok = this.baseModel.isRoot
          ...
          : this.changed.map(function (keys) {
              return this$1.baseModel.getKeypath(this$1.ractive) + '.' + keys.map(escapeKey).join('.')
            })
        ...
        this.baseModel.findMatches(this.keys).forEach(function (model) {
          ...
          if (
            ok.filter(function (k) {
              return keypath.indexOf(k) === 0 || k.indexOf(keypath) === 0
            }).length
          ) {
            count++
            this$1.newValues[keypath] = model.get()
  ...
  function Ractive$on(eventName, callback) {
    ...
    return {
      cancel: function () {
        return this$1.off(eventName, callback)
      },
    }
  }
  ...
  function readLiteral(parser) {
    return (
      readNumberLiteral$1(parser) || readBooleanLiteral(parser) || readStringLiteral(parser) || readObjectLiteral(parser) || readArrayLiteral(parser) || readNumberLiteral(parser)
    )
  }
  ...
  function flattenExpression(expression) {
    ...
    refs = refs.map(function (r) {
      return r.indexOf('...') === 0 ? r.substr(3) : r
    })
          ...
          if (node.spread) {
            ...
            return (
              ...
              (node.o
                ? node.o
                    .map(function (a) {
                      return a.n && a.n.indexOf('...') === 0 ? stringify(a) : '[' + stringify(a) + ']'
                    })
                    .join(',')
                : '') +
              ') )'
  ...
  function processDirective(tokens, parentParser, type) {
    ...
    if (typeof tokens === 'string') {
      ...
      if (type === EVENT && (match = methodCallPattern.exec(tokens))) {
        warnIfDebug(
          parentParser.getContextMessage(pos, "Unqualified method events are deprecated. Prefix methods with '@this.' to call methods on the current Ractive instance.")[2],
        )
    ...
    if (directiveArgs.length && type) {
      warnIfDebug(
        parentParser.getContextMessage(
          parentParser.pos,
          'Proxy events with arguments are deprecated. You can fire events with arguments using "@this.fire(\'eventName\', arg1, arg2, ...)".',
        )[2],
      )
    }
  ...
  function readAttributeOrDirective(parser) {
    ...
    if ((directive = directives[attribute.n])) {
      ...
      if (directive.t === TRANSITION) {
        warnOnceIfDebug(
          '' +
            (directive.v === 't0' ? 'intro-outro' : directive.v === 't1' ? 'intro' : 'outro') +
            " is deprecated. To specify tranisitions, use the transition name suffixed with '-in', '-out', or '-in-out' as an attribute. Arguments can be specified in the attribute value as a simple list of expressions without mustaches.",
        )
      } else if (directive.t === DECORATOR) {
        warnOnceIfDebug(
          "decorator is deprecated. To specify decorators, use the decorator name prefixed with 'as-' as an attribute. Arguments can be specified in the attribute value as a simple list of expressions without mustaches.",
        )
      }
    }
  ...
  function readElement(parser) {
    var start, element, attribute, selfClosing, children, partials, hasPartials, child, closed, pos, remaining, closingTag
  ...
  function readStyle(css) {
    ...
    return css
      ...
      .map(function (rule) {
        return rule.replace(value, function (match, n) {
          return values[n]
        })
      })
  ...
  var BindingFlag = (function (Item) {
    ...
    BindingFlag.prototype.toString = function toString() {
      return ''
    }
  ...
  mutatorMethods.forEach(function (methodName) {
    var method = function () {
      ...
      this._ractive.wrappers.forEach(function (r) {
        if (r.magic) r.magic.locked = true
      })
      ...
      this._ractive.wrappers.forEach(function (r) {
        if (r.magic) r.magic.locked = false
      })
  ...
  var RootModel = (function (Model) {
    ...
    RootModel.prototype.joinKey = function joinKey(key, opts) {
      ...
      if (this.expressions.hasOwnProperty(key)) {
        warnIfDebug(
          'Accessing expression keypaths (' +
            key.substr(1) +
            ') from the instance is deprecated. You can used a getNodeInfo or event object to access keypaths with expression context.',
        )
        return this.expressions[key]
      }

      return this.computations.hasOwnProperty(key) ? this.computations[key] : Model.prototype.joinKey.call(this, key, opts)
    }
  ...
  EventDirective.prototype.bind = function bind() {
    ...
    if (template.x) {
      ...
      this.action =
        typeof template === 'string' // on-click='foo'
          ? template
  ...
  EventDirective.prototype.render = function render() {
    ...
    runloop.scheduleTask(function () {
      return this$1.events.forEach(function (e) {
        return e.listen(this$1)
      }, true)
    })
  }

  EventDirective.prototype.toString = function toString() {
    return ''
  }
  ...
  var Component = (function (Item) {
    ...
    Component.prototype.unrender = function unrender$1(shouldDestroy) {
      ...
      this.liveQueries.forEach(function (query) {
        return query.remove(this$1.instance)
      })
    }
  ...
  Decorator.prototype.handleChange = function handleChange() {
    this.bubble()
  }
  ...
  Decorator.prototype.toString = function toString() {
    return ''
  }
  ...
  function warnAboutAmbiguity(description, ractive) {
    warnOnceIfDebug(
      'The ' + description + ' being used for two-way binding is ambiguous, and may cause unexpected results. Consider initialising your data to eliminate the ambiguity',
      { ractive: ractive },
    )
  }
  ...
  var RadioNameBinding = (function (Binding) {
    ...
    RadioNameBinding.prototype.bind = function bind() {
      ...
      this.nameAttributeBinding = {
        handleChange: function () {
          return (this$1.node.name = '{{' + this$1.model.getKeypath() + '}}')
        },
      }
  ...
  function isBindable(attribute) {
    return attribute && attribute.template.f && attribute.template.f.length === 1 && attribute.template.f[0].t === INTERPOLATOR && !attribute.template.f[0].s
  }
  ...
  function createMapping(item) {
    ...
    if (template.length === 1 && template[0].t === INTERPOLATOR) {
      ...
      if (!item.model) {
        warnOnceIfDebug(
          'The ' + item.name + "='{{" + template[0].r + "}}' mapping is ambiguous, and may cause unexpected results. Consider initialising your data to eliminate the ambiguity",
          { ractive: item.element.instance },
        ) // TODO add docs page explaining item
    ...
    Partial.prototype.forceResetTemplate = function forceResetTemplate() {
      ...
      if (this.inAttribute) {
        doInAttributes(function () {
          return this$1.fragment.resetTemplate(this$1.partialTemplate)
        })
      } else {
  ...
  RepeatedFragment.prototype.destroyed = function destroyed() {
    this.iterations.forEach(function (i) {
      return i.destroyed()
    })
  }
  ...
  RepeatedFragment.prototype.updatePostShuffle = function updatePostShuffle() {
    ...
    var len = this.context.get().length,
      oldLen = this.previousIterations.length
    var i
    var removed = {}
        ...
        setTimeout(function () {
          ...
          while (i--) {
            ...
            if (!CSS_TRANSITIONS_ENABLED || cannotUseCssTransitions[hash]) {
              ...
              interpolator =
                interpolate(parseFloat(originalValue), parseFloat(to[prop])) ||
                function () {
                  return to[prop]
                }
  ...
  function processItems(items, values, guid, counter) {
    ...
    return items
      .map(function (item) {
        ...
        values[placeholderId] = model ? (model.wrapper ? model.wrapper.value : model.get()) : undefined
  ...
  function Ractive$toggle(keypath) {
    ...
    return set(
      this,
      gather(this, keypath).map(function (m) {
        return [m, !m.get()]
      }),
    )
  }
