...
export class Catalog {
  ...
  public isEmpty(): boolean {
    const count = Object.keys(this.simsByLanguage).reduce((acc, langCode) => (acc + op.get(this.simsByLanguage[langCode].length)) as number, 0)
    return count === 0
  }
...
export class Transformer {
  ...
  constructor({ source, bar, handler, verbose = false, workers = 0 }) {
    this.source = source
    this.bar = bar
